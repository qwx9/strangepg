...
flushdraw_(void)
{
	draw(screen, screen->r, viewfb, nil, addpt(viewΔ, view.pan));
...
{
	lockdisplay(display);
	draw(viewfb, viewfb->r, col[Ctext], nil, ZP);
...
resetdraw_(void)
{
	...
	viewΔ = divpt(addpt(subpt(ZP, subpt(screen->r.max, screen->r.min)), viewr.v), 2);
