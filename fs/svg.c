...
/* FIXME: have to do a 2d projection on the screen or sth */
int
exportsvg(char *path)
{
	...
	if((fs = openfs(path, OWRITE)) == nil)
		return -1;
	/* FIXME: ideally done in draw or w/e */
	...
	for(r=rnodes, re=r+dylen(r); r<re; r++){
		...
		p = seprint(buf, buf+sizeof buf,
			"<rect transform=\"translate(%f %f) rotate(%f) scale(%f 1)\" "
	...
	for(e=redges, ee=e+dylen(e); e<ee; e++){
		p = seprint(buf, buf+sizeof buf,
			...
			"style=\"stroke:#cccccc;stroke-opacity:0.5;stroke-width:1\"/>\n",
